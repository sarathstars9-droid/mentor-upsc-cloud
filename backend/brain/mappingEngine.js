...
function getTopicRuleMeta(subject, topic) {
    const raw = SUBJECT_RULES?.[subject]?.topicRules?.[topic];
...
function getMicroRuleMeta(topic, rule) {
    if (!rule) {
        return { name: "", keys: [], node: null };
    }
...
function detectTopic(text, subject) {
    ...
    if (!config) {
        return { topic: "Unmapped Topic", confidence: 0.2, score: 0, node: null };
    }
    ...
    for (const topic of Object.keys(config.topicRules || {})) {
        const meta = getTopicRuleMeta(subject, topic);
        const mergedKeywords = [
            topic,
            ...(meta.aliases || []),
            ...(meta.keywords || [])
        ];
        const score = scoreKeywords(text, mergedKeywords);

        if (score > bestScore) {
            bestScore = score;
            bestTopic = topic;
            bestNode = meta.node || null;
        }
    }
...
function detectMicrotheme(text, topic, existingMicrotheme = "") {
    ...
    for (const rawRule of microRules) {
        const rule = getMicroRuleMeta(topic, rawRule);
        const score = scoreKeywords(text, rule.keys);
        if (score > bestScore) {
            bestScore = score;
            bestMicro = rule.name;
            bestNode = rule.node || null;
        }
    }
...
function fixOneQuestion(q) {
    ...
    const mappedNodeHint = microResult.node || topicResult.node || q.syllabusNodeId || null;
