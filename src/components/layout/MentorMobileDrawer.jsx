    ...
    <>
      ...
      <aside className={`mos-drawer-v2 ${open ? "open" : ""}`} role="dialog" aria-modal="true">
        ...
        <div className="mos-nav-scroll-v2">
          {navGroups.map((group) => (
            <section className="mos-nav-section-v2" key={group.key} aria-labelledby={`mos-sec-m-${group.key}`}>
              <div id={`mos-sec-m-${group.key}`} className="mos-nav-section-label-v2">{group.label}</div>
              <div>
