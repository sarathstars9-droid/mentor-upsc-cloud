...
export default function MainsGS2Page() {
  ...
  const themeCounts = useMemo(() => Object.fromEntries(
    THEMES.map((theme) => [theme.id, questions.filter((q) => q.theme === theme.id).length]),
  ), [questions]);
    ...
    <div className="mpw-page">
      <main className="mpw-shell">
        <PremiumPaperHero
          ...
          subtitle="Polity, Governance, Social Justice & International Relations — browse the paper, study PYQ patterns, and start focused answer writing."
          stats={[
        ...
        {showAnalysis ? <div className="mpw-insight-area"><PYQAnalysisPanel questions={questions} /></div> : null}
        {showTrends ? <div className="mpw-insight-area"><YearwiseTrendsPanel questions={questions} /></div> : null}
        {showYearTest ? <div className="mpw-insight-area"><YearTestPanel questions={questions} onStart={handleStart} /></div> : null}
        ...
        <section aria-labelledby="gs2-question-list-title">
          <div className="mpw-list-heading">
            <div>
              <div className="mpw-eyebrow">Question Library</div>
              <h2 id="gs2-question-list-title">{activeTheme === "all" ? "All GS2 Questions" : activeTheme}</h2>
            </div>
            ...
            <div className="mpw-question-list">
              {pagedQuestions.map((q) => (
                <PremiumQuestionCard
                  ...
                  q={q}
                  onStart={handleStart}
                  themeLabel={THEMES.find((theme) => theme.id === q.theme)?.label || q.theme}
                />
              ))}
            </div>
          ) : null}
          {!loading && !error && questions.length > 0 && sortedFiltered.length === 0 ? <PremiumEmptyState /> : null}
          {!loading && !error && questions.length === 0 ? <PremiumEmptyState title="No questions loaded" copy="Check that the tagged Mains data is available from the backend." /> : null}

          {!loading && !error && sortedFiltered.length > 0 ? (
            <PremiumPagination page={currentPage} pageCount={pageCount} onChange={(next) => { setPage(next); window.scrollTo({ top: 0, behavior: "smooth" }); }} />
          ) : null}
        </section>
      </main>
    </div>
